                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4" style="display: flex; justify-content: center;">
                                    <div class="btn-group">
                                        ...
                                        <a href="{{ route('validationMails.index') }}" class="btn btn-primary" style="margin-bottom: 0; cursor: pointer; height: 38px;">
                                            Validation
                                        </a>
                        ...
                        <table id="datatable" class="table table-bordered table-hover display select" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th>List(s)</th>
                                    <th>Theme</th>
                                    {{-- <th>Open/Unsub/Bounce/Clicks</th> --}}
                                    <th>Status</th>
                                    {{-- <th>Recuring</th> --}}
                                    <th>Author</th>
                                    <th>Date</th>
                            ...
                            <tbody>
                            @foreach($newsletterhistories as $newsletterhistory)
                            <tr data-id="{{ $newsletterhistory->id }}">
                                <td style="font-weight: bold;">{{ $newsletterhistory->title }}</td>
                                <td>{{ $newsletterhistory->list }}</td>
                                <td>{{ $newsletterhistory->theme }}</td>
                                {{-- <td>45.34% / 26.36% / 16.65% / 67</td> --}}
                                <td>Sent(1 times)</td>
                                {{-- <td>No</td> --}}
                                <td>{{ $newsletterhistory->author }}</td>
                                <td>{{ $newsletterhistory->updated_at }}</td>
                                <td>{{ $newsletterhistory->attachments }}</td>
                                <td>
                                    <a href="{{ route('newsletters.show',$newsletterhistory->id) }}" title="Show">
                                        <button class="btn-info"><i class="mdi mdi-eye "></i></button>
                                    </a>
                                    <button class="btn-danger btn-delete-record" title="Delete" data-id="{{ $newsletterhistory->id }}"><i class="mdi mdi-delete"></i></button>
                                </td>
                            </tr>
                            @endforeach
                            </tbody>
                ...
                $('#delete-all').click(function () {
                    ...
                    if(flag){
                        swal({
                            ...
                            $.ajax({
                                url: "{{ route('newsletters.multiple-delete') }}",
                                type: 'DELETE',
                                ...
                                success: function (data) {
                                    if (data['status']==true) {
                                        ...
                                        swal(
                                            'Deleted!',
                                            'Your newsletters have been deleted.',
                                            'success'
                                        );
                                    } else {
